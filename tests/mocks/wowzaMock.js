...
module.exports = function(networkClientMock) {
    ...
    _.chain(files).filter(function (file) {
        ...
        networkClientMock.read.withArgs({
            url : url.resolve('http://vidsegsec-a.akamaihd.net/dc-0/m/pa-live-publish2/vLive/smil:12345_all.smil/', file),
            timeout : 10000
        }).returns(readFile(file));
    });

    var masterPlaylist = readFile('playlist.m3u8');
    networkClientMock.read.withArgs('http://mediaserverhost/vLive/smil:12345_all.smil/playlist.m3u8').returns(masterPlaylist);
    ...
    networkClientMock.read.withArgs({
        url: 'http://vidsegsec-a.akamaihd.net/dc-0/m/pa-live-publish2/vLive/smil:12345_all.smil/chunklist_b987136.m3u8',
        timeout: 10000
