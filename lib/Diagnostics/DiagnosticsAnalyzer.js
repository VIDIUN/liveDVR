...
class DiagnosticsAnalyzer extends EventEmitter{
    ...
    _checkBackupStreamOnlyAlert(entryObject) {
        let alertObj;
        if (entryObject.recordStatus !== vidiunTypes.VidiunRecordStatus.DISABLED) {
        ...
        if (!entryObject.isRedundancy && (entryObject.serverType === vidiunTypes.VidiunEntryServerNodeType.LIVE_BACKUP)) {
            this._logger.info(alertObj.msg);
            this.alertsToSend.push(alertObj);
        }
        else {
