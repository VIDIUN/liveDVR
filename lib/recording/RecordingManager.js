...
// todo Assumptions:
    ...
    constructor() {
        ...
        TaskManager.register(vidiunTypes.VidiunEntryServerNodeType.LIVE_CLIPPING_TASK, clippingTaskTemplateName, this.handleNewLiveClippingTask);
    ...
    handleRecordingEntries() {
        ...
        function isRecordingFinish(recordingSession) {
            ...
            if (recordingSession.entryObject.recordingStatus == vidiunTypes.VidiunRecordingStatus.PAUSED)
            {
                logger.info("[%s-%s] recording is paused", recordingSession.entryId, recordingSession.recordedEntryId);
                return false;
            }
            else if (recordingSession.entryObject.recordingStatus == vidiunTypes.VidiunRecordingStatus.STOPPED)
            {
            ...
            return recordingEntrySession.serializedActionsPromise
                .finally(() => { // check first that all chunks are completed
                    ...
                    return qio.write(stampFilePath, recordingDuration)
                        .then(() => {
                            if (recordingEntrySession.serverType !== vidiunTypes.VidiunEntryServerNodeType.LIVE_PRIMARY) {
                        ...
                        .then(() => {
                            ...
                            delete this.recordingList[entryId];
                            if (resetRecordedEntry && (entryObject.serverType == vidiunTypes.VidiunEntryServerNodeType.LIVE_PRIMARY))
                            {
    ...
    handleNewLiveClippingTask(task)
    {
        ...
        return qio.makeTree(newPath).then(() => {
            ...
            if (playList.clipFrom >= playList.clipTo)
            {
                ...
                backendClient.updateEntryServerNodeStatus(task.id, vidiunTypes.VidiunEntryServerNodeStatus.ERROR);
            ...
            return backendClient.updateEntryServerNodeStatus(task.id, vidiunTypes.VidiunEntryServerNodeStatus.TASK_QUEUED);
        }).catch((err) => {
