...
class TaskManager {
    ...
    _pushMessageReceived(msg)
    {
        ...
        BackendClient.getEntryServerNode(task.id).then(entryServerNode => {
            if (entryServerNode && entryServerNode.status == VidiunTypes.VidiunEntryServerNodeStatus.TASK_PENDING)
    ...
    {
        let filter = new vidiunVO.VidiunEntryServerNodeFilter();
        filter.serverNodeIdEqual = serverNodeId;
        filter.statusIn = VidiunTypes.VidiunEntryServerNodeStatus.TASK_PENDING;
