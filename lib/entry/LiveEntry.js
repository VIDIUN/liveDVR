...
const VidiunLiveStatus = require('../vidiun-client-lib/VidiunTypes').VidiunEntryServerNodeStatus;
...
function LiveEntry(entryObject) {
    ...
    this.recovered = entryObject.liveStatus && entryObject.liveStatus === VidiunLiveStatus.PLAYABLE;
    ...
    this.recordingEnable =  entryObject.recordStatus != vidiunTypes.VidiunRecordStatus.DISABLED  &&
...
LiveEntry.prototype.start = function() {
    ...
    return this.playlistGenerator.initializeStart()
        ...
        .then(()=> {
            if (this.recordingEnable && this.entryObject.recordingStatus == vidiunTypes.VidiunRecordingStatus.ACTIVE) {
...
LiveEntry.prototype.entryObjectUpdated = function(entry)
{
    ...
    if (entry.viewMode == vidiunTypes.VidiunViewMode.ALLOW_ALL
        && origViewMode != vidiunTypes.VidiunViewMode.ALLOW_ALL)
    {
        this._startDVR(entry);
        this._removeClipTo().then (()=>{this._updateIsPlayableUser(true)});
    }
    if (entry.viewMode == vidiunTypes.VidiunViewMode.PREVIEW
        && origViewMode != vidiunTypes.VidiunViewMode.PREVIEW)
    {
    ...
    if (entry.recordingStatus == vidiunTypes.VidiunRecordingStatus.ACTIVE
        && origRecordingStatus != vidiunTypes.VidiunRecordingStatus.ACTIVE)
    {
        this._startRecording(entry);
    }
    else if (entry.recordingStatus == vidiunTypes.VidiunRecordingStatus.STOPPED
        && origRecordingStatus != vidiunTypes.VidiunRecordingStatus.STOPPED)
    {
...
LiveEntry.prototype._stopRecording = function(entry)
{
    ...
    const resetRecordedEntryId = (this.entryObject.recordStatus != vidiunTypes.VidiunRecordStatus.APPENDED);
