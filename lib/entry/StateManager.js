...
module.exports = function(liveEntry) {
    ...
    function onbroadcast(event, from, to) {
        return checkNewSession()
            ...
            .then(function() {
                ...
                return (liveEntry.entryObject.liveStatus == VidiunLiveStatus.PLAYABLE) ? Q.resolve() : sync.exec(function() {
                    pushManager.setObjectEnabled(liveEntry.entryId, false);
                    return backendClient.registerEntryInDatabase(liveEntry.entryObject, VidiunLiveStatus.BROADCASTING, event)
                        .then((entryObj) => {
                ...
                .then(function() {
                    return sync.exec(function() {
                        return updateStreamInfo(currTime)
                            .finally(() => {
                                pushManager.setObjectEnabled(liveEntry.entryId, false);
                                return backendClient.registerEntryInDatabase(liveEntry.entryObject, VidiunLiveStatus.PLAYABLE, event)
                                    .then((newEntryObj) => {
    ...
    function checkStreamRedundancy(entryServerNodesList) {
        if (entryServerNodesList.length > 1) {
            ...
            return sn.status !== vidiunTypes.VidiunEntryServerNodeStatus.MARKED_FOR_DELETION;
        }
        return false;
    }
